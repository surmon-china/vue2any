...
function renderSSRClass ( staticClass, dynamic) {
  ...
  if (instance.transfer.validate(res)) {
...
const _renderAttr = (key, value) => {
  ...
  if (instance.transfer.validate(value)) {
    return ` ${key}="${value}"`
  } else {
...
const _renderStringList = (list, render) => {
  ...
  if (instance.transfer.validate(list)) {
    ...
    const listData = instance.transfer.getDataFromEcho(list)
    let forEachCode = instance.transfer.foreach.before(listData, params[0], params[1])
    forEachCode += renderStringList(1, render)
    forEachCode += instance.transfer.foreach.after
    // console.warn('那我转了？？？', forEachCode)
    return forEachCode
  } else {
...
exports.renders = function(instanceContext, vmContext) {
  if (!instance) {
    vmInstance = vmContext
    instance = instanceContext
    vtaGenerate = require('./generate')
  }
  return {
    _ssrEscape: _escape,
