...
function renderSSRClass (
  ...
  if (vueToAny.transfer.validate(res)) {
    return res ? ` class="${res}"` : res
  }
  return res === '' ? res : (" class=\"" + (cachedEscape(res)) + "\"")
}
...
function renderSSRStyle (
  ...
  var res = genStyle(style);
  if (vueToAny.transfer.validate(res)) {
    return res === '' ? res : (" style=" + (JSON.stringify(res)))
  } else {
    return res === '' ? res : (" style=" + (JSON.stringify(cachedEscape(res))))
  }
}
...
// 样式渲染
const _renderSSRStyle = (staticStyle, dynamic, extra) => {
  return renderSSRStyle(staticStyle, dynamic, extra)
}

// class 渲染
const _renderSSRClass = (staticClass, dynamic) => {
  console.log('_renderSSRClass', staticClass, dynamic)
  return renderSSRClass(staticClass, dynamic)
}

// 字符串转义
const _escape = s => {
  if (vueToAny.transfer.validate(s)) {
    // 如果要解析目标语言语句，则是需要过滤或不过滤
    s = vueToAny.transfer.echoStripTagsFromEcho(s)
    return s
  } else {
    return escape(s)
  }
...
// 属性渲染
const _renderAttr = (key, value) => {
  // console.log('_renderAttr', key, value)
  if (vueToAny.transfer.validate(value)) {
    return ` ${key}="${value}"`
  } else {
    return renderAttr(key, value)
  }
}

// 列表渲染
const _renderStringList = function(list, render) {
  // 如果所循环数据为目标语言，则输出目标语言
  if (vueToAny.transfer.validate(list)) {

    // 被循环数据
    const listData = vueToAny.transfer.getDataFromEcho(list)

    // v-for 的参数列表
    const params = getParameterNames(render)

    // 构造 List foreach
    let forEachCode = vueToAny.transfer.foreach.before(listData, params[0], params[1])

    // 计算子元素
    const _s = _escape
    const _ssrEscape = _escape
    const _ssrStyle = _renderSSRStyle
    const _ssrClass = _renderSSRClass

    // console.log('parses', parses)
    params.forEach(keyword => {
      render = parses.parseVariable.bind(vueToAny)(render, keyword)
    })

    eval(`render = ${render}`)
    const renderResult = render()
    // console.log('renderResult', renderResult, 'forEachCode', forEachCode)
    forEachCode += renderResult
    forEachCode += vueToAny.transfer.foreach.after
    return forEachCode
  } else {
    return renderStringList(list, render)
  }
}

exports.renders = function(vueToAnyContext) {
  vueToAny = vueToAnyContext
  return {
    _ssrEscape: _escape,
    _ssrNode: _renderStringNode$1,
    _ssrList: _renderStringList,
    _l: _renderStringList,
    _ssrAttr: _renderAttr,
    _ssrAttrs: renderAttrs$1,
    _ssrDOMProps: renderDOMProps$1,
    _ssrClass: renderSSRClass,
    _ssrStyle: _renderSSRStyle
  } 
}
